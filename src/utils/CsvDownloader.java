package utils;

import java.net.*;
import java.nio.file.*;

public class CsvDownloader {
    private static final Path DOWNLOAD_LOCATION_PATH = Paths.get("src", "data");

    public static void downloadCsvFile(String downloadLink, String fileName) throws Exception {
        try {
            if (!Files.exists(DOWNLOAD_LOCATION_PATH)) {
            Files.createDirectories(DOWNLOAD_LOCATION_PATH);
            }
            Path destination = getFileDestination(fileName);
            URL url = new URI(downloadLink).toURL();
            Files.copy(url.openStream(), destination);
            System.out.println("Downloaded CSV file successfully.");
        } catch (Exception e) {
            throw new RuntimeException(e.getMessage(), e);
        }
    }

        private static Path getFileDestination(String fileName) {
        return DOWNLOAD_LOCATION_PATH.resolve(fileName + ".csv");
    }
}
